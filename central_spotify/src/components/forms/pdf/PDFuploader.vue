    ...
    <v-card elevation-1>
      ...
      <v-card-text>
        ...
        <v-btn v-if="url && (!loading)" @click="openFile()" block color="primary" text>
          <v-icon class="mx-3">mdi-file-pdf</v-icon>
          Abrir aquivo
        </v-btn>
        <v-btn v-if="!disabled" :disabled="loading" block @click="showFileChooser()" color="primary" text>
          <v-icon class="mx-3"> {{url ? 'mdi-folder-refresh' : 'mdi-folder-multiple-image'}} </v-icon>
...
export default {
  ...
  methods: {
    async uploadFile(file, fileBase64, bucket, subfolder){
      try {
        this.loading = true
        let fileUploaded = await upload.pdf(file, fileBase64, bucket, subfolder)
        this.url = this.getUrlPDF(fileUploaded.fileURLKey, this.bucket) 
  ...
  mounted(){
    if(this.filename) this.url = this.getUrlPDF(this.filename, this.bucket)
  },
  computed: {
  }
};
